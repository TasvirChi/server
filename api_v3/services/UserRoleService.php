 ...
 * @subpackage services
 */
class UserRoleService extends BorhanBaseService
{
	...
	public function addAction(BorhanUserRole $userRole)
	{
		...
		try { PermissionPeer::checkValidPermissionsForRole($userRole->permissionNames, $this->getPartnerId());	}
			...
			if ($code == kPermissionException::PERMISSION_NOT_FOUND) {
				throw new BorhanAPIException(BorhanErrors::PERMISSION_NOT_FOUND, $e->getMessage());
			}
		}
	 ...
	 * @return BorhanUserRole The retrieved user role object
	 * 
	...
	public function getAction($userRoleId)
	{
		...
		if (!$dbUserRole) {
			throw new BorhanAPIException(BorhanErrors::INVALID_OBJECT_ID, $userRoleId);
		}
	 ...
	 * @action update
	 * @param int $userRoleId The user role's unique identifier
	 * @param BorhanUserRole $userRole The user role's unique identifier
	 * @return BorhanUserRole The updated user role object
	 *
	 * @throws BorhanErrors::INVALID_OBJECT_ID
	 * @throws BorhanErrors::PERMISSION_NOT_FOUND
	 */	
	public function updateAction($userRoleId, BorhanUserRole $userRole)
	{
		...
		if (!is_null($userRole->permissionNames) && !($userRole->permissionNames instanceof BorhanNullField)) {
			try { PermissionPeer::checkValidPermissionsForRole($userRole->permissionNames, $this->getPartnerId());	}
			catch (kPermissionException $e) {
				$code = $e->getCode();
				if ($code == kPermissionException::PERMISSION_NOT_FOUND) {
	 ...
	 * @action delete
	 * @param int $userRoleId The user role's unique identifier
	 * @return BorhanUserRole The deleted user role object
	 *
	 * @throws BorhanErrors::INVALID_OBJECT_ID
	 * @throws BorhanErrors::ROLE_IS_BEING_USED
	 */		
	public function deleteAction($userRoleId)
	{
		...
		try {
			...
			if ($code == kPermissionException::ROLE_IS_BEING_USED) {
				throw new BorhanAPIException(BorhanErrors::ROLE_IS_BEING_USED);
			}
			throw $e;			
		}	
		$dbUserRole->save();
	 ...
	 * @return BorhanUserRoleListResponse The list of user role objects
	 */
	public function listAction(BorhanUserRoleFilter  $filter = null, BorhanFilterPager $pager = null)
	{
		if (!$filter)
			$filter = new BorhanUserRoleFilter();
	 ...
	 * @action clone
	 * @param int $userRoleId The user role's unique identifier
	 * @return BorhanUserRole The duplicate user role object
	 * 
	 * @throws BorhanErrors::INVALID_OBJECT_ID
	 */
