...
class UnitTestsGenerator extends ClientGeneratorFromPhp
{
	 ...
	 */
	protected function writeBeforeService(BorhanServiceActionItem $serviceActionItem)
	{
		...
		$this->writeBase(" */");
		$this->writeBase("abstract class {$serviceClass}TestBase extends BorhanApiTestCase");
		$this->writeBase("{");
	 ...
	 * @param BorhanServiceReflector $serviceReflector
	 */
	protected function writeSetUpServiceFunction(BorhanServiceActionItem $serviceReflector)
	{
		$this->writeTest("	/**");
	 ...
	 * @param BorhanServiceReflector $serviceReflector
	 */
	protected function writeService(BorhanServiceActionItem $serviceReflector)
	{
		...
		BorhanLog::debug("Service name [$serviceName] id [$serviceId] class [$serviceClass]");
		
		foreach($actions as $action => $actionReflector)
		{
		    /* @var $actionReflector BorhanActionReflector */
			$actionInfo = $actionReflector->getActionInfo();
			BorhanLog::debug("Action name [" . $actionReflector->getActionName() . "]");
				
			if($actionInfo->serverOnly)
			{
				BorhanLog::debug("Action name [" . $actionReflector->getActionName() . "] server only");
				continue;
			}

			if (strpos($actionInfo->clientgenerator, "ignore") !== false)
			{
				BorhanLog::debug("Action name [" . $actionReflector->getActionName() . "] ignored");
				continue;
			}
	...
	protected function writeAfterService(BorhanServiceActionItem $serviceReflector)
	{
		...
		{
			$testPath = realpath(dirname(__FILE__) . '/../') . "/tests/api/BorhanPlugins/{$serviceClass}_{$serviceName}";
		}
	 ...
	 * @param BorhanParamInfo $outputTypeReflector
	 */
	protected function setOutputData(BorhanParamInfo $outputTypeReflector, &$testParams, &$testValues, $isBase = false, &$validateValues = null)
	{
		...
		else
		{
			...
			{
				/* @var $actionParamProperty BorhanPropertyInfo */
	 ...
	 */
	protected function setTestParamsAndValues(BorhanParamInfo $actionParam, &$testParams, &$testValues, &$validateValues, $isBase)
	{
		$paramType = $actionParam->getType();
		$paramName = $actionParam->getName();
		BorhanLog::debug("paramName [$paramName] paramType [$paramType]");
	...
	{
		BorhanLog::debug("Service id [$serviceId] name [$serviceName] action [$action]");
		
		/* @var $outputTypeReflector BorhanParamInfo */
		if($outputTypeReflector && $outputTypeReflector->isFile())
		{
			BorhanLog::debug("Service id [$serviceId] name [$serviceName] action [$action] is file");
			return;
		}
		...
		//BorhanLog::info("Generates action [$serviceName.$action]");
		...
		if($isBase && $outputType)
		{
			...
			$serviceReflector = BorhanServiceReflector::constructFromServiceId($serviceId);
	...
	private function writeToFile($fileName, $contents, $overwrite = true)
	{
		...
		{
			//BorhanLog::info("File [$fileName] already exists not writing data");
			return;
		}
