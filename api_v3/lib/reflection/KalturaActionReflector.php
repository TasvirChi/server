 ...
 * BorhanServiceMap.cache, so long as the service class can be found. Internal use only.
 * 
 * @package api
 * @subpackage v3
 */
class BorhanActionReflector extends BorhanReflector
{
	/**
	 * @var string
	 */
	...
	public function __construct($serviceId, $actionId, $serviceCallback)
	{
		...
		if(function_exists('apc_fetch'))
		{
			...
			if($fetchFromAPCSuccess && $actionFromCache[BorhanServicesMap::SERVICES_MAP_MODIFICATION_TIME] == BorhanServicesMap::getServiceMapModificationTime())
			{
	...
	public function getActionInfo ( )
	{
		if (is_null($this->_actionInfo))
		{
			...
			$this->_actionInfo = new BorhanDocCommentParser($reflectionMethod->getDocComment());
		}
	...
	public function getActionClassInfo ()
	{
		...
		{
			$reflectionClass = new ReflectionClass($this->_actionClass);
			$this->_actionClassInfo = new BorhanDocCommentParser($reflectionClass->getDocComment());
		}
	 ...
	 */
	public function initService(BorhanDetachedResponseProfile $responseProfile = null)
	{
		$instance = $this->getServiceInstance();
	...
	protected function cacheReflectionValues()
	{
		...
		$servicesMapLastModTime = BorhanServicesMap::getServiceMapModificationTime();
		
		$cacheValue = array(
			BorhanServicesMap::SERVICES_MAP_MODIFICATION_TIME => $servicesMapLastModTime, 
