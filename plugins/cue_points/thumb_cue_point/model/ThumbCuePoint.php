...
class ThumbCuePoint extends CuePoint implements IMetadataObject
{
	...
	public function copyAssets( entry $toEntry, ThumbCuePoint $toCuePoint, $adjustedStartTime = null )
	{
		...
		if ( ! $timedThumbAsset )
		{
			BorhanLog::info("Can't retrieve timedThumbAsset with id: {$this->getAssetId()}");
			return;
		}
		...
		BorhanLog::log("Saved cue point [{$toCuePoint->getId()}] and timed thumb asset [{$toTimedThumbAsset->getId()}]");
	}
