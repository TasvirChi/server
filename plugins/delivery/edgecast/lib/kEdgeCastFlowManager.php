...
class kEdgeCastFlowManager implements kObjectDeletedEventConsumer
{
    ...
    private static function purgeAssetFromEdgeCast(asset $asset)
	{
	    ...
	    try {
	        ...
	        BorhanLog::err('Cannot get asset URL for asset id ['.$asset->getId().'] - '.$e->getMessage());
	    }
	...
	{        
        ...
        if (!$edgeCastParams) {
            BorhanLog::err('Partner ['.$partner->getId().'] does not have any edge cast parameters configured');
            return false;
        }
        ...
        BorhanLog::info('Curl results: '.print_r($curlResults, true));
	}
