 ...
 * @subpackage admin
 */
class AdminConsoleGalleryAction extends BorhanApplicationPlugin
{
	...
	{
		return array(Borhan_Client_Enum_PermissionName::SYSTEM_ADMIN_ENTRY_INVESTIGATION);
	}
	...
	public function doAction(Zend_Controller_Action $action)
	{
		...
		if($partnerId > 0)
		{
			...
			else
			{
				...
				$uiConfFilter->objTypeEqual = Borhan_Client_Enum_UiConfObjType::PLAYER_V3;
				$uiConfFilter->orderBy = Borhan_Client_Enum_UiConfOrderBy::CREATED_AT_DESC;
				$uiConfPager = new Borhan_Client_Type_FilterPager();
				$uiConfPager->pageSize = 1;
				$uiConfList = $adminConsolePlugin->uiConfAdmin->listAction($uiConfFilter, $uiConfPager);
				/* @var $uiConfList Borhan_Client_AdminConsole_Type_UiConfAdminListResponse */
				if(count($uiConfList->objects))
					$action->view->uiConf = reset($uiConfList->objects);
			}
			
			if($action->view->uiConf)
				$action->view->swfUrl = "/index.php/bwidget/wid/_{$partnerId}/cache_st/" . time() . "/uiconf_id/" . $action->view->uiConf->id;
		}
	}
}
