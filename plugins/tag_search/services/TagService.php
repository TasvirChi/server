 ...
 */
class TagService extends BorhanBaseService
{   
     ...
     */
    public function searchAction (BorhanTagFilter $tagFilter, BorhanFilterPager $pager = null)
    {
        if (!$tagFilter)
        {
            $tagFilter = new BorhanTagFilter();
        }
        ...
        $searchResponse = new BorhanTagListResponse();
        $searchResponse->objects = BorhanTagArray::fromDbArray($tags, $this->getResponseProfile());
    ...
    public function deletePendingAction ()
    {
		...
		if (!$count)
		{
			BorhanLog::info ('No tags pending for deletion.');
			return 0;
		}
	...
	public function indexCategoryEntryTagsAction ($categoryId, $pcToDecrement, $pcToIncrement)
	{
		...
		$c->add(TagPeer::PRIVACY_CONTEXT, $pcToDecrementArray, BorhanCriteria::IN);
		TagPeer::setUseCriteriaFilter(false);
