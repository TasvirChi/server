...
abstract class Form_EventNotificationTemplateConfiguration extends Infra_Form
{
	...
	/**
	 * @var Borhan_Client_EventNotification_Enum_EventNotificationTemplateType
	 */
	...
	abstract protected function addTypeElements(Borhan_Client_EventNotification_Type_EventNotificationTemplate $eventNotificationTemplate);
	...
	/**
	 * @param Borhan_Client_Type_StringValue|Borhan_Client_Type_BooleanValue $field
	 * @return string
	 */
	protected function getValueDescription($field)
	{
		if($field instanceof Borhan_Client_Type_EvalStringField || $field instanceof Borhan_Client_Type_EvalBooleanField)
			return 'Code: ' . $field->code;
	
		if($field->value)
			return $this->getDescriptionHtml($field->value);
		
		return $field->getBorhanObjectType();
	}
	
	protected function getParameterDescription(Borhan_Client_EventNotification_Type_EventNotificationParameter $parameter)
	{
	...
	protected function getConditionDescription(Borhan_Client_Type_Condition $condition)
	{
		if($condition->description)
			return $this->getDescriptionHtml($condition->description);
			
		if($condition instanceof Borhan_Client_EventNotification_Type_EventFieldCondition)
			return $this->getDescriptionHtml($this->getValueDescription($condition->field));
			
		return $condition->getBorhanObjectType();
	}
	...
	{
		if(!($object instanceof Borhan_Client_EventNotification_Type_EventNotificationTemplate))
			return;
		...
		if($object instanceof Borhan_Client_EventNotification_Type_EventNotificationTemplate)
		{
			...
			foreach($properties as $property => $value)
			{
				...
				if(preg_match('/^userParameterKey_(\d+)$/', $property, $matches))
				{
					...
					{
						$field = new Borhan_Client_Type_StringValue();
						$field->value = $properties["userParameterValue_{$index}"];
						
						$userParameter = new Borhan_Client_EventNotification_Type_EventNotificationParameter();
						$userParameter->value = $field;
					}
					else
					{
						$userParameter = new Borhan_Client_EventNotification_Type_EventNotificationArrayParameter();
						...
						foreach($properties as $subProperty => $subValue)
						{
							...
							{
								$string = new Borhan_Client_Type_String();
							...
							elseif(preg_match("/^userParameterAllowedValue_{$index}_(\d+)$/", $subProperty, $subMatches))
							{
								...
								$string = new Borhan_Client_Type_StringValue();
								$string->value = $subValue;
	 ...
	 * @param Borhan_Client_EventNotification_Type_EventNotificationTemplate $eventNotificationTemplate
	 */
	public function resetUnUpdatebleAttributes(Borhan_Client_EventNotification_Type_EventNotificationTemplate $eventNotificationTemplate)
	{
	...
	public function finit(Borhan_Client_EventNotification_Type_EventNotificationTemplate $eventNotificationTemplate)
	{
	...
	public function init()
	{
		...
		$eventType = new Borhan_Form_Element_EnumSelect('event_type', array(
			'enum' => 'Borhan_Client_EventNotification_Enum_EventNotificationEventType',
			'disabled'		=> true,
			'label'			=> 'Event type:',
		));
		
		$eventObjectType = new Borhan_Form_Element_EnumSelect('event_object_type', array(
			'enum' => 'Borhan_Client_EventNotification_Enum_EventNotificationEventObjectType',
			'disabled'		=> true,
	...
	/**
	 * @param Borhan_Client_HttpNotification_Type_EventNotificationParameter $parameter
	 */
	protected function addUserParameter(Borhan_Client_EventNotification_Type_EventNotificationParameter $parameter)
	{
		$elements = array();
		...
		if($parameter instanceof Borhan_Client_EventNotification_Type_EventNotificationArrayParameter)
		{
			$values = array();
			if(is_array($parameter->values))
			{
				...
				{
					/* @var $value Borhan_Client_Type_String */
					$values[] = $value->value;
				}
			}
			...
			{
				/* @var $allowedValue Borhan_Client_Type_StringValue */
