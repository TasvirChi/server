...
class SymantecScanDirectEngine extends SymantecScanEngine
{
	...
	protected function sendCommandToScanEngine($command)
	{
		...
		if (!$engineSocket)
		{
			...
			BorhanLog::err("Failed to create socket: [$errorCode] " . socket_strerror($errorCode));
			return false;
		}
		...
		if (!socket_connect($engineSocket, self::SCAN_ENGINE_HOST, self::SCAN_ENGINE_PORT))
		{
			...
			BorhanLog::err("Failed to connect to scan engine: [$errorCode] " . socket_strerror($errorCode));
		...
		{
			$errorCode = socket_last_error($engineSocket);
			BorhanLog::err("Failed to send data to socket server: [$errorCode] " . socket_strerror($errorCode));
			socket_close($engineSocket);
			return false;
		}
		...
		for (;;)
		{
			...
			{
				$errorCode = socket_last_error($engineSocket);
				BorhanLog::err("Failed to recv data from socket server: [$errorCode] " . socket_strerror($errorCode));
				break;
			}
...
";
		...
		for ($scanAttempts = 0; $scanAttempts < $this->maxScanRetries; sleep($this->sleepBetweenScanRetries))
		{
			...
			BorhanLog::info("Buffer received from scan engine: $response");

			if (!kString::beginsWith($response, 'ICAP/1.0 '))
			{
				BorhanLog::err("Response does not start with ICAP/1.0");
				$scanAttempts++;
				continue;
			}
			...
			case '200':
			case '204':
				return BorhanVirusScanJobResult::FILE_IS_CLEAN;
				
			case '201':
				return BorhanVirusScanJobResult::FILE_WAS_CLEANED;

			case '205':
			case '403':
				return BorhanVirusScanJobResult::FILE_INFECTED;
				
			case '502':
				$errorDescription = 'Scan engine failed to access source file ['.$filePath.']';
				return BorhanVirusScanJobResult::SCAN_ERROR;		// no reason to retry
			...
			default:	// incl: 500 - internal error
				BorhanLog::err("Got invalid scan status $statusCode");
				$scanAttempts++;				
				continue;
			}
		}
