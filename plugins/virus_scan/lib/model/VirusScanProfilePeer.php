...
class VirusScanProfilePeer extends BaseVirusScanProfilePeer
{
	...
	public static function getSuitableProfile($entryId)
	{
		...
		if (!$entry)
		{
			BorhanLog::err('Cannot find entry with id ['.$entryId.']');
			return null;
		}
		...
		if (!$profiles)
		{
			BorhanLog::info('No virus scan profiles found for partner ['.$entry->getPartnerId().']');
			return null;
		}
		...
		foreach ($profiles as $profile)
		{
			...
			{
				BorhanLog::info('Returning profile with id ['.$profile->getId().']');
				return $profile;
			}
