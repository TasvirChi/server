...
$borhanRootPath = realpath(__DIR__ . '/../../../');

require_once "$borhanRootPath/tests/monitoring/BorhanMonitorResult.php";
if($argc == 1)
{
	echo "usage...";
}

$systemConfig = parse_ini_file("$borhanRootPath/configurations/system.ini");
...
$xml = implode("\n", $outputLines);
$monitorResult = BorhanMonitorResult::fromXml($xml);
...
if($monitorResult->errors)
{
	...
	foreach($monitorResult->errors as $error)
	{
		...
		switch($error->level)
		{
		    case BorhanMonitorError::EMERG:
		    case BorhanMonitorError::ALERT:
		    case BorhanMonitorError::CRIT:
		    	$exitCode = NAGIOS_CODE_CRITICAL;
		    	break;
		    	
		    case BorhanMonitorError::WARN:
		    	$exitCode = max($exitCode, NAGIOS_CODE_WARNING);
		    	break;
		    	
		    case BorhanMonitorError::NOTICE:
		    case BorhanMonitorError::INFO:
		    case BorhanMonitorError::DEBUG:
		    	break;
		    	
		    case BorhanMonitorError::ERR:
		    default:
