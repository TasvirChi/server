 ...
 */
class SphinxCriterion extends BorhanCriterion implements IBorhanIndexQuery
{
	...
	protected function needsBrackets()
	{
		...
		if($parentCriterion instanceof BorhanCriterion)
	...
	protected function getStringMatchClause($sphinxField, $comparison, $value)
	{
		...
		switch($comparison)
		{
			...
			case BorhanCriteria::IN_LIKE_ORDER:
				$vals = is_array($value) ? $value : explode(',', $value);
	...
	/* (non-PHPdoc)
	 * @see BorhanCriterion::apply()
	 */
	public function apply(IBorhanIndexQuery $query)
	{
		...
		// Can apply criterion
		BorhanLog::info("Applies criterion [$field]");
	
		if(!$objectClass::hasIndexFieldName($field))
		{
			BorhanLog::debug("Skip criterion[$field] has no sphinx field");
			return false;
		}
		...
		{
			BorhanLog::debug("Skip criterion[$field] unhandled comparison [$comparison]");
			return false;
		}
		...
		BorhanLog::debug("Attach criterion[$field] as sphinx field[$sphinxField] of type [$type] and comparison[$comparison] for value[$valStr]");
		...
		if($type == IIndexable::FIELD_TYPE_STRING)
		{
			...
			BorhanLog::debug("Add match criterion[$field] as sphinx field[$sphinxField] of type [$type] match [$match] line [" . __LINE__ . "]");
			$this->addMatch($match);
		}
		else
		{
			$condition = $this->getNonStringClause($sphinxField, $type, $comparison, $value);
			BorhanLog::debug("Add condition criterion[$field] as sphinx field[$sphinxField] of type [$type] condition [$condition] line [" . __LINE__ . "]");
		...
		foreach($clauses as $index => $clause)
		{
			...
			{
				BorhanLog::debug("Clause [" . $clause->getColumn() . "] is not Sphinx criterion");
				return false;
			}
			
			if(!$clause->apply($this))
			{
				BorhanLog::debug("Failed to apply clause [" . $clause->getColumn() . "]");
				return false;
			}
		}
		...
		if (count($this->conditionClause))
		{
			...
			if (!$this->hasOr())
			{
				...
				BorhanLog::debug("Add where criterion[$field] as sphinx field[$sphinxField] of type [$type] where [$where] line [" . __LINE__ . "]");
			...
			{
				BorhanLog::debug("Simplifying expression [$attributesClause] => [{$expSimplifications[$attributesClause]}]");
	...
	public function addOr(Criterion $criterion)
	{
		...
		BorhanLog::debug("Add OR criterion field [$addedField] to current field [$currentField]");
		...
		{		
			...
			{
				BorhanLog::debug("Current sphinx field [$currentSphinxField] and added sphinx field [$addedField]");
				...
				{
					BorhanLog::debug("Current type [$currentType] and added type [$addedType]");
