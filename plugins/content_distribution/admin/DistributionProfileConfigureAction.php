 ...
 * @subpackage admin
 */
class DistributionProfileConfigureAction extends BorhanApplicationPlugin
{
	protected $client;
	...
	public function doAction(Zend_Controller_Action $action)
	{
		...
		$contentDistributionPlugin = Borhan_Client_ContentDistribution_Plugin::get($this->client);
		...
		try
		{
			...
			$form = null;
			$profileClass = 'Borhan_Client_ContentDistribution_Type_DistributionProfile';
			
			if($providerType == Borhan_Client_ContentDistribution_Enum_DistributionProviderType::GENERIC)
			{
				$form = new Form_GenericProviderProfileConfiguration($partnerId, $providerType, $distributionProfile);
				$profileClass = 'Borhan_Client_ContentDistribution_Type_GenericDistributionProfile';
			}
			elseif($providerType == Borhan_Client_ContentDistribution_Enum_DistributionProviderType::SYNDICATION)
			{
				$form = new Form_SyndicationProviderProfileConfiguration($partnerId, $providerType, $distributionProfile);
				$profileClass = 'Borhan_Client_ContentDistribution_Type_SyndicationDistributionProfile';
			}
			else
			{
				$form = BorhanPluginManager::loadObject('Form_ProviderProfileConfiguration', $providerType, array($partnerId, $providerType, $distributionProfile));
				$profileClass = BorhanPluginManager::getObjectClass($profileClass, $providerType);
			}
			if(!$form)
			{
	...
	protected function populateForm(Form_DistributionConfiguration $form, Borhan_Client_ContentDistribution_Type_DistributionProfile $distributionProfile, Borhan_Client_Type_FlavorParamsListResponse $flavorParamsResponse)
	{
