...
abstract class Form_ConfigurableProfileConfiguration extends Form_ProviderProfileConfiguration
{
	...
	public function getObject($objectType, array $properties, $add_underscore = true, $include_empty_fields = false)
	{
		...
		foreach($itemXpathsToExtend as $key => $val)
		{
			if ((string)$key == 'includeCategoryInMrss'){
				$categoryFeaturesToExtend[] = Borhan_Client_Enum_ObjectFeatureType::METADATA;
				continue;
			}
			else if ((string)$key == 'includeCategoryParentInMrss'){
				$categoryFeaturesToExtend[] = Borhan_Client_Enum_ObjectFeatureType::ANCESTOR_RECURSIVE;
				continue;
			}
			else if ($val){	
				$temp = new Borhan_Client_Type_ExtendingItemMrssParameter();
				$temp->xpath = $val;
				$temp->identifier = new Borhan_Client_Type_EntryIdentifier();
				$temp->identifier->identifier = Borhan_Client_Enum_EntryIdentifierField::ID;
				$temp->identifier->extendedFeatures = "";
				$temp->extensionMode = Borhan_Client_Enum_MrssExtensionMode::APPEND;
				$object->itemXpathsToExtend [] = $temp;
			}
		}
		
		if (count($categoryFeaturesToExtend)){
			$categoryFeaturesToExtend[] = Borhan_Client_Enum_ObjectFeatureType::CUSTOM_DATA;
			$temp = new Borhan_Client_Type_ExtendingItemMrssParameter();
			$temp->xpath = '//category';
			$temp->identifier = new Borhan_Client_Type_CategoryIdentifier();
			$temp->identifier->identifier = Borhan_Client_Enum_CategoryIdentifierField::FULL_NAME;
			$temp->identifier->extendedFeatures = implode(',', $categoryFeaturesToExtend);
			$temp->extensionMode = Borhan_Client_Enum_MrssExtensionMode::REPLACE;
			$object->itemXpathsToExtend [] = $temp;
		}
		return $object;
	}
	...
	protected function addItemXpathsToExtend($itemXpathsToExtend)
	{
		if (count($itemXpathsToExtend) == 0)
			$itemXpathsToExtend = array(new Borhan_Client_Type_ExtendingItemMrssParameter());
		...
		{
			/* @var $itemXPath Borhan_Client_Type_ExtendingItemMrssParameter */
			//if it a category identifier
			if ($itemXPath->identifier instanceof Borhan_Client_Type_CategoryIdentifier){
				/* @var $identifier Borhan_Client_Type_CategoryIdentifier */
				$identifier = $itemXPath->identifier;
				//if the parameters are set exactly as the admin console sets.
				if ($itemXPath->xpath == '//category' && $itemXPath->extensionMode == Borhan_Client_Enum_MrssExtensionMode::REPLACE
					&& $identifier->identifier == Borhan_Client_Enum_CategoryIdentifierField::FULL_NAME){
					foreach (explode(',', $identifier->extendedFeatures) as $extendedFeature){
						if ($extendedFeature == Borhan_Client_Enum_ObjectFeatureType::METADATA){
							$extendCategory = true;
						}
						elseif ($extendedFeature == Borhan_Client_Enum_ObjectFeatureType::ANCESTOR_RECURSIVE){
