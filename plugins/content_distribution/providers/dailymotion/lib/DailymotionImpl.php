...
class DailyMotionImpl 
{
	...
	private function call($method, $args = array()) {
		BorhanLog::info ( "Call [$method] args [" . print_r ( $args, true ) . "]" );
		$result = $this->api->call ( $method, $args );
		BorhanLog::info ( "Result [" . print_r ( $result, true ) . "]" );
	...
	public function uploadSubtitle($remoteVideoId, BorhanDailymotionDistributionCaptionInfo $captionInfo) {
	...
	public function updateSubtitle($remoteSubtitleId, BorhanDailymotionDistributionCaptionInfo $captionInfo) {
		$url = $this->api->uploadFile ( $captionInfo->filePath );
		$args = array ();
		$args ['url'] = $url;
		$args ['language'] = $captionInfo->language;
	...
	private function getCaptionFormate($format) {
		switch ($format) {
			case BorhanDailymotionDistributionCaptionFormat::TT :
				return 'TT';
			case BorhanDailymotionDistributionCaptionFormat::SRT :
				return 'SRT';
			case BorhanDailymotionDistributionCaptionFormat::STL :
				return 'STL';
		}
	}
}
