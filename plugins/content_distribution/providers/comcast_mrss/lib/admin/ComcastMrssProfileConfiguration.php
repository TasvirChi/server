...
class Form_ComcastMrssProfileConfiguration extends Form_ConfigurableProfileConfiguration
{
	public function getObject($objectType, array $properties, $add_underscore = true, $include_empty_fields = false)
	{
		...
		if ($itemsNode)
		{
			...
			foreach($itemNodes as $itemNode)
			{
				...
				$keyVal = new Borhan_Client_Type_KeyValue();
	...
	{
		if ($distributionProfile instanceof Borhan_Client_ComcastMrssDistribution_Type_ComcastMrssDistributionProfile)
		{
			if ($this->getValue('c_platform_tv_series_field'))
			{
				...
				if ($element)
				{
					...
					if ($restrictionNode)
					{
						...
						{
							BorhanLog::info('Updating metadata profile ['.$profileId.'] field ['.$fieldName.'] with new list of values');
							...
							$metadataPlugin->metadataProfile->update($profileId, new Borhan_Client_Metadata_Type_MetadataProfile(), $doc->saveXML());
						}
						else
						{
							BorhanLog::info('No fields update is required for metadata ['.$profileId.'] field ['.$fieldName.']');
						}
					}
				}
			}
