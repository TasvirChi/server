...
class IdeticDistributionProvider extends ConfigurableDistributionProvider
{
	 ...
	 * @return DOMDocument
	 */
	public static function generateXML($entryId, BorhanIdeticDistributionJobProviderData $providerData)
	{
		...
		if(!$mrss)
		{
			BorhanLog::err("No MRSS returned for entry [$entryId]");
			return null;
		}
		...
		if(!$xml->loadXML($mrss))
		{
			BorhanLog::err("Could not load MRSS as XML for entry [$entryId]");
			return null;
		}
		...
		{
			BorhanLog::err("XSL file not found [$xslPath]");
			return null;
		}
		...
		foreach($varNodes as $varNode)
		{
			...
			if($name && $providerData->$name)
			{
				...
				BorhanLog::info("Set variable [$name] to [{$providerData->$name}]");
			}
		}
		...
		{
			BorhanLog::err("Schema validation failed");		
			return null;
		}
