...
class WidgetController extends Zend_Controller_Action
{
	public function listAction()
	{
		...
		$uiConfFilter->orderBy = Borhan_Client_Enum_UiConfOrderBy::CREATED_AT_DESC;
		...
		$paginatorAdapter = new Borhan_FilterPaginatorWithPartnerLoader($adminConsolePlugin->uiConfAdmin, "listAction", null, $uiConfFilter);
	...
	public function createAction()
	{
		...
		if ($request->isPost())
		{
			...
			{
				$uiConf = $form->getObject('Borhan_Client_AdminConsole_Type_UiConfAdmin', $request->getPost());
	...
	public function editAction()
	{
		...
		if ($request->isPost())
		{
			...
			if ($form->isValid($request->getParams()))
			{
				$uiConfUpdate = $form->getObject('Borhan_Client_AdminConsole_Type_UiConfAdmin', $request->getPost());
	...
	public function bcwEditorAction()
	{
		$request = $this->getRequest();
		$this->view->bcwEditorVersion = "v1.2.0";
		$this->view->bcwBaseUrl = Infra_ClientHelper::getServiceUrl() . '/flash/bcweditor/';
	...
	{
		$uiConfFilter = new Borhan_Client_Type_UiConfFilter();
		...
		switch($filterType)
		{
			...
			case 'by-partner-name':
				$partnerFilter = new Borhan_Client_Type_PartnerFilter();
				...
				$partnerFilter->orderBy = Borhan_Client_Enum_PartnerOrderBy::ID_DESC;
				$client = Infra_ClientHelper::getClient();
	...
	{
		$types = array();
		$reflectionClass = new ReflectionClass('Borhan_Client_Enum_UiConfObjType');
		$constants = $reflectionClass->getConstants();
		foreach($constants as $name => $value)
		{
			$types[$value] = 'Borhan_Client_Enum_UiConfObjType::' . $name;
		}
		return $types;
	}
}