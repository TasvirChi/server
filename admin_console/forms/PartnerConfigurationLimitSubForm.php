...
class Form_PartnerConfigurationLimitSubForm extends Zend_Form_SubForm
{
	...
	public function populateFromObject(Form_PartnerConfiguration $form, Borhan_Client_SystemPartner_Type_SystemPartnerConfiguration $partnerConfiguration, Borhan_Client_SystemPartner_Type_SystemPartnerLimit $object, $add_underscore = true)
	{
		$isPermitted = true;
		if(count($this->requiredPartnerPermissions))
		{
			...
			if($partnerConfiguration->permissions && count($partnerConfiguration->permissions))
			{
				...
				{
					if(isset($requiredPartnerPermissions[$permission->name]) && $permission->status == Borhan_Client_Enum_PermissionStatus::ACTIVE)
					{
	...
	public function getObject($objectType, array $properties, $add_underscore = true, $include_empty_fields = false)
	{
		$object = null;
		if($this->withOverage)
			$object = new Borhan_Client_SystemPartner_Type_SystemPartnerOveragedLimit();
		else
			$object = new Borhan_Client_SystemPartner_Type_SystemPartnerLimit();
