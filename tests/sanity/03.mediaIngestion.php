...
$resource->url = $clientConfig->serviceUrl . 'content/templates/entry/data/borhan_logo_animated_blue.flv';
...
while ($createdEntry)
{
	...
	if($createdEntry->status != BorhanEntryStatus::IMPORT && $createdEntry->status != BorhanEntryStatus::PRECONVERT)
	{
...
/* @var $entryContextDataResult BorhanEntryContextDataResult */
...
if(isset($headers['x-borhan']) && strpos($headers['x-borhan'], 'error-') === 0)
{
	echo "Entry [$createdEntry->id] manifest error: " . $headers['x-borhan'] . "\n";
	exit(-1);
}
echo "OK\n";
...
if(isset($headers['x-borhan']) && strpos($headers['x-borhan'], 'error-') === 0)
{
	echo "Entry [$createdEntry->id] download error: " . $headers['x-borhan'] . "\n";
	exit(-1);
}
echo "OK\n";
...
if(isset($headers['x-borhan']) && strpos($headers['x-borhan'], 'error-') === 0)
{
	echo "Entry [$createdEntry->id] raw error: " . $headers['x-borhan'] . "\n";
	exit(-1);
}
echo "OK\n";
...
if(isset($headers['x-borhan']) && strpos($headers['x-borhan'], 'error-') === 0)
{
	echo "Entry [$createdEntry->id] thumbnail error: " . $headers['x-borhan'] . "\n";
	exit(-1);
}
echo "OK\n";
