...
class criteriaFilter
{
	...
	private function copyCriteriaConstraints($fromCriteria, $toCriteria)
	{
		...
		foreach ( $columns as $column )
		{
			...
			if ($filterCriterion instanceof BorhanCriterion && !$filterCriterion->isEnabled())
			{
				BorhanLog::debug("Skip criterion[" . $filterCriterion->getColumn() . "] comparison [ " . $filterCriterion->getComparison() . " ] with disabled tag [ " . print_r($filterCriterion->getTags(), true) . " ]");
				continue;
			}
	...
	private function addClauses ( Criteria $criteriaToFilter , Criterion $filterCriterion , Criterion $criterion  )
	{
		...
		{
			if($clause instanceof BorhanCriterion && !$clause->isEnabled())
				continue;
