 ...
 * @deprecated
 */
class viewPartnersAction extends borhanSystemAction
{
...
//		$c->addAnd ( PartnerPeer::ID , $exclude_list , Criteria::NOT_IN );
	...
	private static function getPartnersStats ( $ids , $from_date, $to_date )
	{
		...
		$results  = self::executeQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_BDP , true );
		...
		foreach($results as $resultset)
	    {
	    	...
	    	if ( $sub_act == PartnerActivity::PARTNER_SUB_ACTIVITY_BDP_PLAYS ) $stats[$partner_id]["plays"] = $value;
	    	elseif ( $sub_act == PartnerActivity::PARTNER_SUB_ACTIVITY_BDP_VIEWS ) $stats[$partner_id]["views"] = $value;
	    }	  
	...
	private static function activeSite ( $connection , &$stats ,  $ids_str , $to_date , $delta , $title )
	{
		...
		$results  = self::executeActiveSiteQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_BDP );
	...
	private static function executeActiveSiteQuery ( $connection , $ids_str , $dateFilter , $activity , $sub_activity = false )
	{
		$min_plays = 1;
		$min_views = 10;
		// PARTNER_SUB_ACTIVITY_BDP_PLAYS = 201
		// PARTNER_SUB_ACTIVITY_BDP_VIEWS = 202
		$group_by = "PARTNER_ID";
		if ( $sub_activity ) $group_by .= ",SUB_ACTIVITY";
...
//	print_r ( $partner_groups->partner_group_list );
		...
		if ( $group->field == "text" )
		{
			...
			// remove from the list -  $partner_list & $borhan_related
			$list_of_ids = array();
