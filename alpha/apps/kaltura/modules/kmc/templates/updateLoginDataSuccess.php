...
<head>
	...
	button { margin: 0 auto 5px; padding: 0 28px 0 0; height: 25px; border: 0; font: normal 11px arial,sans-serif; color:#2B2B2B; line-height: normal; overflow: visible; background: url(/lib/images/bmc/bmc_sprite.png) no-repeat -72px -152px; cursor: pointer; }
	button span { height:20px; padding: 4px 0 0 28px; margin: 1px 1px 0 0; float:left; white-space:nowrap; background:transparent url(/lib/images/bmc/bmc_sprite.png) no-repeat scroll 0 -153px;}
	...
	var forceBMCHttps = <?php echo ($forceBMCHttps) ? 'true' : 'false'; ?>;
	// Check if our parent has the same protocol and host name and then check
	// for top / window objects
	if( forceBMCHttps && top != window && top.location.hostname != window.location.hostname ) { 
