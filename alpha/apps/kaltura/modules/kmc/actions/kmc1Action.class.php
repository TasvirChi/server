 ...
 * @subpackage BMC
 */
class bmc1Action extends borhanAction
{
...
//				$this->ui_conf_height = 0 ; // $ui_conf->getHeight();
		...
		if ($this->partner_id !== NULL)
		{
			...
			bmcUtils::redirectPartnerToCorrectBmc($partner, $this->ks, $this->uid, $this->screen_name, $this->email, 1);
		...
		if($partner)
		{
			...
			if(kConf::get('bmc_content_enable_commercial_transcoding'))
			{
		...
		*/
		...
		$this->bmc_content_version 	= 'v1.1.11';
		$this->bmc_account_version 	= 'v1.1.7';
		$this->bmc_appstudio_version 	= 'v1.2.4';
		$this->bmc_rna_version 		= 'v1.0.5';
		$this->bmc_dashboard_version 	= 'v1.0.1';
	...
	private function getUiconfList($tag = 'player')
	{
		...
		$crit_default_swf_url = $c->getNewCriterion(uiConfPeer::SWF_URL, '%/bdp3/%bdp3.swf', Criteria::NOT_LIKE);
