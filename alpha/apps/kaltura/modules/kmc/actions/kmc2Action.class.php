 ...
 * @subpackage BMC
 */
class bmc2Action extends borhanAction
{
...
//				$this->ui_conf_height = 0 ; // $ui_conf->getHeight();
		...
		if ($this->partner_id !== NULL)
		{
			...
			bmcUtils::redirectPartnerToCorrectBmc($partner, $this->ks, $this->uid, $this->screen_name, $this->email, 2);
		...
		{
			if ($partner->getLiveStreamEnabled() && $partner->getBmcVersion() == 3)
			{
				$this->enable_live_streaming = 'true';
			}
		}

		// this is Andromeda bmc2Action - following are irrelevant so we set them to false & empty
...
*/
		...
		$this->bmc_content_version 	= 'v2.1.6.1';
		$this->bmc_account_version 	= 'v2.1.2.3';
		$this->bmc_appstudio_version 	= 'v2.0.4';
		$this->bmc_rna_version 		= 'v1.1.3';
		$this->bmc_dashboard_version 	= 'v1.0.10';
