...
FROM 
	...
	FROM
			borhandw.dwh_hourly_user_usage u
			WHERE
			{OBJ_ID_CLAUSE}
	...
	FROM
		borhandw.dwh_hourly_user_usage u JOIN (SELECT kuser_id, MAX(date_id) date_id FROM borhandw.dwh_hourly_user_usage u WHERE {OBJ_ID_CLAUSE} AND partner_id = {PARTNER_ID} and date_id <= {TO_DATE_ID} GROUP BY kuser_id) total
