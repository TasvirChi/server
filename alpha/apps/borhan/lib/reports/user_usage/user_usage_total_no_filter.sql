...
FROM 
	...
	FROM
		borhandw.dwh_hourly_user_usage u JOIN (SELECT kuser_id, MAX(date_id) date_id FROM borhandw.dwh_hourly_user_usage WHERE partner_id = {PARTNER_ID} and date_id <= {TO_DATE_ID} GROUP BY kuser_id) total
		ON u.kuser_id = total.kuser_id AND u.date_id = total.date_id) total
