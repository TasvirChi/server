...
(
	...
	(	SELECT	partner_status_id STATUS, partner_name, created_at, partner_package,
		...
		FROM borhandw.dwh_dim_partners dim_partner 
		LEFT JOIN borhandw.dwh_hourly_partner aggr_partner  
	...
	(	SELECT dim_partner.partner_id partner_id, 	IFNULL(SUM(count_bandwidth_kb), 0) count_bandwidth,
		...
		FROM borhandw.dwh_dim_partners dim_partner 
		LEFT JOIN borhandw.dwh_hourly_partner_usage hourly_partner_usage 
...
(
	...
	FROM borhandw.dwh_hourly_partner aggr_partner RIGHT JOIN borhandw.dwh_dim_partners dim_partner ON (aggr_partner.partner_id = dim_partner.partner_id AND aggr_partner.date_id <= {TO_DATE_ID})
