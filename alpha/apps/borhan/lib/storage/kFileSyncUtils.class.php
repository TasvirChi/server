...
class kFileSyncUtils implements kObjectChangedEventConsumer, kObjectAddedEventConsumer
{
	...
	{
		BorhanLog::debug("key [$key], fetch_from_remote_if_no_local [$fetch_from_remote_if_no_local]");
		list ( $file_sync , $local ) = self::getReadyFileSyncForKey( $key , $fetch_from_remote_if_no_local , false  );
		if ( ! $file_sync )
		{
			BorhanLog::info("FileSync not found");
			return false;
		}
		else
		{
		...
		BorhanLog::info("file_exists? [$file_exists] took [".(microtime(true)-$startTime)."] path [".$file_sync->getFullPath()."]");
	...
	public static function getContentsByFileSync ( FileSync $file_sync , $local = true , $fetch_from_remote_if_no_local = true , $strict = true )
	{
		if ( $local )
		{
			...
			if ( file_exists ( $real_path ) )
			{
				...
				BorhanLog::info("file was found locally at [$real_path] fgc took [".(microtime(true) - $startTime)."]");

				return $contents;
			}
			else
			{
				BorhanLog::info("file was not found locally [$full_path]");
	...
	{
		BorhanLog::debug("key [$key], strict [$strict]");
		...
		if(!$file_sync || !$local)
		{
			...
			BorhanLog::err("FileSync not found");
			return null;
		}
		...
		if(!is_dir($real_path))
		{
			BorhanLog::info("directory was not found locally [$real_path]");
		...
		BorhanLog::info("directory was found locally at [$real_path]");
	...
	public static function file_get_contents ( FileSyncKey $key , $fetch_from_remote_if_no_local = true , $strict = true , $max_file_size = 0 )
	{
		...
		if ($cacheStore)
		{
			...
			if ($result)
			{
				BorhanLog::info("returning from cache, key [$cacheKey] size [".strlen($result)."]");
				return $result;
			}
		}

		BorhanLog::debug("key [$key], fetch_from_remote_if_no_local [$fetch_from_remote_if_no_local], strict [$strict]");
		...
		if($file_sync)
		{
			...
			{
				BorhanLog::err('FileSync size [' . $file_sync->getFileSize() . '] exceeds the limit [' . $max_file_size . ']');
				return null;
			}
			...
			{
				BorhanLog::info("saving to cache, key [$cacheKey] size [".strlen($result)."]");
	...
	public static function file_put_contents ( FileSyncKey $key , $content , $strict = true )
	{
		...
		if($res)
		{
			...
			BorhanLog::err("File Sync key $key already exists");
		}
		else
		{
			BorhanLog::info("File Sync doesn't exist");
		}
	...
	public static function moveToFile ( FileSyncKey $source_key , $target_file_path, $delete_source = true , $overwrite = true)
	{
		...
		{
			BorhanLog::notice('could not load ready file sync for key '.$source_key);
			return false;
		}
		...
		{
			BorhanLog::debug("Target file [$target_file_path] exists");
			if(!$overwrite)
			{
				BorhanLog::notice("target [$target_file_path] exists, not overwriting");
				return false;
			}
		...
		{
			BorhanLog::info("$target_file_path file doesnt exist");
		}
		...
		if($copyResult)
		{
			...
			BorhanLog::info("successfully copied file to [$target_file_path] and updated fileSync");
			return true;
		}
		else
		{
			BorhanLog::info("copy failed - not changing filesync");
			return false;
		}
	}
	...
	{
		BorhanLog::debug("move file to directory: [$temp_file_path] to key [$directory_key]");
		...
		else
		{
			...
			if(!$dirFullPath)
			{
				...
				BorhanLog::info("Generated new path [$dirFullPath]");
			}
			...
			{
				BorhanLog::info("Creating directory [$dirFullPath] for file");
		...
		BorhanLog::debug("temp_file_path [$temp_file_path](" . filesize($temp_file_path) . ") destination_file_path [$destination_file_path](" . filesize($destination_file_path) . ")");
	...
	{
		BorhanLog::debug("move file: [$temp_file_path] to key [$target_key], ");
		...
		if($existsFileSync)
		{
			...
			{
				BorhanLog::err("file already exists");
			}
		}
		...
		if(!$targetFullPath)
		{
			...
			BorhanLog::info("Generated new path [$targetFullPath]");
		}
		...
		{
			BorhanLog::info("$temp_file_path file exists");
		}
		else
		{
			BorhanLog::info("$temp_file_path file doesnt exist");
		}
		...
		{
			BorhanLog::err("could not move file from [$temp_file_path] to [{$targetFullPath}]");
	...
	{
		BorhanLog::debug("copy file: [$temp_file_path] to key [$target_key], ");
...
/**
	...
	/**
	 * Get the internal from borhan data centers only FileSync object by its key
	 *
	 * @param FileSyncKey $key
	 * @return FileSync
	 */
	...
	public static function getReadyFileSyncForKey ( FileSyncKey $key , $fetch_from_remote_if_no_local = false , $strict = true , $resolve = true )
	{
		...
		if ( $file_sync_list == null )
		{
			...
			{
				BorhanLog::notice("FileSync was not found");
		...
		{
			if ($local)
				BorhanLog::info("FileSync was found locally");
			else
				BorhanLog::info("FileSync was found but doesn't exists locally");
		...
		{
			BorhanLog::info("exact FileSync was not found");
			return array ( null , false );
		}
	}
	...
	{
		BorhanLog::debug("key [$key], strict[$strict], already_exists[$alreadyExists]");
	...
	public static function createPendingExternalSyncFileForKey(FileSyncKey $key, StorageProfile $externalStorage, $isDir = false)
	{
		...
		BorhanLog::debug("key [$key], externalStorage [$externalStorageId]");
		...
		if($externalStorage->getProtocol() == StorageProfile::STORAGE_BORHAN_DC)
		{
	...
	{
		BorhanLog::debug("target_key [$target_key], source_key [$source_key]");
		// TODO - see that if in strict mode - there are no duplicate keys -> update existing records AND set the other DC's records to PENDING
		$dc = kDataCenterMgr::getCurrentDc();
		$dc_id = $dc["id"];
		...
		if (!$file_sync_list)
		{
			BorhanLog::notice("Warning: no source. target_key [$target_key], source_key [$source_key] ");
			return null;
		}
	 ...
	 * @param FileSyncKey $key
	 * @param bool $strict
	 * @param bool $fromBorhanDcsOnly
	 * @return string
	 */
	public static function deleteSyncFileForKey( FileSyncKey $key , $strict = false , $fromBorhanDcsOnly = false)
	{
		if ( !$key )
		{
			if ( $strict )
		...
		foreach($file_sync_list as $file_sync)
		{
			...
			else
			{
				...
				{
					BorhanLog::notice("The file sync [" . $file_sync->getId() . "] is associated with [" . $file_sync->getLinkCount() . "] links and won't be deleted");
					return null;
				}
				else
				{
	...
	{
		BorhanLog::debug("FileSync id [" . $file_sync->getId() . "]" );
	...
	{
		BorhanLog::debug('Deleting old file_sync versions for ['.$newFileSync->getId().']');
		if (kConf::hasParam('num_of_old_file_sync_versions_to_keep'))
		{
			$keepCount = kConf::get('num_of_old_file_sync_versions_to_keep');
