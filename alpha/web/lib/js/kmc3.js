...
/* bmc and bmc.vars defined in script block in bmc2success.php */
	...
	// bmc.vars.quickstart_guide = "/content/docs/pdf/BMC_Quick_Start_Guide__Butterfly.pdf#";
	bmc.vars.quickstart_guide = "/content/docs/pdf/BMC3_Quick_Start_Guide.pdf#"; // cassiopea
	...
	bmc.functions = {
		...
		openBcw : function(ks, conversion_profile) {
			...
			var params = {
				...
				quality: "high",
//				wmode: "opaque" ,
				movie: bmc.vars.service_url + "/bcw/ui_conf_id/" + bmc.vars.bcw_uiconf
			};
...
//				wmode				: "opaque"
			...
			flashvars : {
				...
				enableCustomData	: bmc.vars.enable_custom_data,
		...
		dashboard : {
			...
			flashvars : {
				...
				uploadDocLink		: bmc.vars.quickstart_guide + "page=3",
				embedDocLink		: bmc.vars.quickstart_guide + "page=5",
				customizeDocLink	: bmc.vars.quickstart_guide + "page=52" // bf=37
			}
		},
		content : {
			swf_url : bmc.vars.flash_dir + "/bmc/content/" + bmc.vars.versions.content + "/content.swf",
			flashvars : {
				moderationBDPVersion : "v3.3.4",
				drillDownBDPVersion  : "v3.3.4",
				moderationUiconf	: bmc.vars.content_moderate_uiconf,
				drilldownUiconf		: bmc.vars.content_drilldown_uiconf,
				...
				enableLiveStream	: bmc.vars.enable_live,
				sampleFileUrl		: "/content/docs/csv/borhan_batch_upload_andromeda.csv",
				metadataViewUiconf	: bmc.vars.metadata_view_uiconf
			}
		},
		appstudio : {
			...
			flashvars : {
				entryId					: bmc.vars.appStudioExampleEntry ,
				"playlistAPI.kpl0Name"	: "playlist1",
				"playlistAPI.kpl0Url"	: '',
				"playlistAPI.kpl1Name"	: "playlist2",
				"playlistAPI.kpl1Url"	: '',
				inapplicationstudio		: "true",
				Appstudiouiconfid		: bmc.vars.appstudio_uiconfid,
				//bdpUrl					: bmc.vars.flash_dir + "/bdp3/v3.3.4/bdp3.swf",
				...
				templatesXmlUrl			: bmc.vars.appstudio_templatesXmlUrl || "",
				enableAds				: bmc.vars.enableAds,
				enableCustomData		: bmc.vars.enable_custom_data
//				widget_id				: "_" + bmc.vars.partner_id
			}
		},
		...
		reports : {
			...
			flashvars : {
				drillDownBdpVersion	: "v3.3.4",
				drillDownBdpUiconf	: bmc.vars.reports_drilldown,
				serverPath			: bmc.vars.service_url
			}
		}
	}
	bmc.utils.mergeJson(bmc.modules.appstudio.flashvars,{ "playlistAPI.kpl0Url"	: bmc.modules.appstudio.playlist_url + bmc.vars.appStudioExamplePlayList0, "playlistAPI.kpl1Url" : bmc.modules.appstudio.playlist_url + bmc.vars.appStudioExamplePlayList1 });
//	bmc.modules.studio = bmc.modules.appstudio;
...
//			alert("doPreviewEmbed: id="+id+", name="+name+", description="+description+", is_playlist="+is_playlist+", uiconf_id="+uiconf_id);
			...
			if(id != "multitab_playlist") {
				...
				description = bmc.utils.escapeQuotes(description); // @todo: move to "// JW" block
...
//			console.log("uiconf_details="+uiconf_details);
			...
			else if(bmc.vars.silverlight) {
				embed_code = bmc.preview_embed.buildSilverlightEmbed(id, name, is_playlist, uiconf_id);
				preview_player = embed_code.replace('{KS}','ks=' + bmc.vars.ks);
				embed_code = embed_code.replace('{KS}','');
				embed_code = embed_code.replace("{ALT}", ((bmc.vars.whitelabel) ? "" : "<br/>" + bmc.preview_embed.embed_code_template.borhan_links));
			}
			else {
			...
			var modal_html = '<div id="modal"><div id="titlebar"><a id="close" href="#close"></a>' +
							 ...
							 ((typeof live_bitrates == "object") ? bmc.preview_embed.buildLiveBitrates(name,live_bitrates) : '') +
//							 ((id == "multitab_playlist") ? '' : bmc.preview_embed.buildSelect(id, name, description, is_playlist, uiconf_id)) +
							 ...
							 (bmc.vars.jw ? jw_nomix_box_html : '') +
							 '<div id="player_wrap">' + preview_player + '</div>' +
							 (bmc.vars.jw ? jw_options_html : '') +
							 ((bmc.vars.silverlight || live_bitrates) ? '' : bmc.preview_embed.buildRtmpOptions()) +
...
//			alert(modal_html);
			...
			$("#player_select").change(function(){
				bmc.preview_embed.doPreviewEmbed(id, name, description, is_playlist, this.value, live_bitrates);
			});
		}, // doPreviewEmbed
		...
		buildSilverlightEmbed : function(id, name, is_playlist, uiconf_id) {
			var uiconf_details = bmc.preview_embed.getUiconfDetails(uiconf_id,is_playlist),
			cache_st = bmc.preview_embed.setCacheStartTime(),
			embed_code = bmc.preview_embed.embed_code_template.silverlight;
			embed_code = embed_code.replace("{WIDTH}",uiconf_details.width);
			embed_code = embed_code.replace("{HEIGHT}",uiconf_details.height);
			embed_code = embed_code.replace("{HOST}",bmc.vars.host);
			embed_code = embed_code.replace("{CACHE_ST}",cache_st);
			embed_code = embed_code.replace("{VER}",uiconf_details.swfUrlVersion); // sl
			embed_code = embed_code.replace("{PARTNER_ID}",bmc.vars.partner_id);
//			embed_code = embed_code.replace("{CDN_HOST}",bmc.vars.cdn_host); // sl
...
//			alert("flavor_details.asset_id="+flavor_details.asset_id);
		...
		embed_code_template :	{
			object_tag :	'<object id="borhan_player" name="borhan_player" type="application/x-shockwave-flash" allowFullScreen="true" ' +
			...
			silverlight :	'<object data="data:application/x-silverlight-2," type="application/x-silverlight-2" width="{WIDTH}" height="{HEIGHT}"> <param name="source" ' +
							'value="http://{HOST}/index.php/bwidget/cache_st/{CACHE_ST}/wid/_{PARTNER_ID}/uiconf_id/{UICONF_ID}/nowrapper/1/a/a.xap" />' +
//							'value="http://{HOST}/flash/slp/v{VER}/BorhanPlayer.xap?widget_id=_{PARTNER_ID}&host={HOST}&cdnHost={CDN_HOST}&uiconf_id={UICONF_ID}" />' +
		...
		buildBorhanEmbed : function(id, name, description, is_playlist, uiconf) {
...
//			alert("cache_st = " + cache_st);
			...
			if(!bmc.vars.jw) { // more efficient to add "&& !bmc.vars.silverlight" (?)
				...
				if(bmc.vars.embed_code_delivery_type == "rtmp") {
					embed_code = embed_code.replace("{FLASHVARS}", "streamerType=rtmp&amp;streamerUrl=" + bmc.vars.rtmp_host + "&amp;rtmpFlavors=1&{FLASHVARS}"); // rtmp://rtmpakmi.borhan.com/ondemand
				}
			}
...
//				console.log(uiconf_details.swf_version); alert("uiconf_details.swf_version logged");
			...
			embed_code = embed_code.replace("{MEDIA}", "video");	// to be replaced by real media type once doPreviewEmbed (called from within BMC>Content) starts passing full entry object			embed_code = embed_code.replace(/{ENTRY_ID}/gi, (is_playlist ? "-1" : id));
...
//								bmc.utils.mergeJson(bmc.editors.flashvars, { "entry_id" : entry_id }),
		...
		flashvars: {
			...
			"terms_of_use"	: bmc.vars.terms_of_use,
			"disableurlhashing" : bmc.vars.disableurlhashing,
