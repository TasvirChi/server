<?php
	...
	*/
	...
	if($argc < 4){
        BorhanLog::debug ("Usage: [fileName] [quality] [realRun]");
	...
	if ($dryRun)
        	BorhanLog::debug('dry run --- in order to save, give realRun as a third parameter');
	...
	foreach($partnerIds as $partnerId){
		...
		if (!$partner)
		{
			BorhanLog::debug("ERR1 - partner $partnerId wasn\'t found");
			continue;
		}
	
		$partner->setDefThumbQuality($quality);
		BorhanLog::debug("saving partner id $partnerId");
		$partner->save();
	}
