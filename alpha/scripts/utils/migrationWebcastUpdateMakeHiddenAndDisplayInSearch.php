...
$dryRun = ($argv[2] == "debug");
if ($dryRun)
    BorhanLog::debug('this is a dry run!');

foreach ($partners as $partnerId)
{
    $partnerId = trim($partnerId);
    BorhanLog::debug("running script for partner [" . $partnerId . "]");
    $partner = PartnerPeer::retrieveByPK($partnerId);
    if(!$partner)
    {
        BorhanLog::warning("partner [ " . $partnerId . "] does not exist. Skipping it");
        continue;
    }
    ...
    BorhanLog::debug("got " . count($entries) . " live entries for partner " . $partnerId);
    foreach ($entries as $liveEntry)
    {
        BorhanLog::debug("Starting to work on Entry [" . $liveEntry->getId() . "]");
        ...
        if ($recordingOptions)
        {
            ...
            BorhanLog::debug("calling setShouldMakeHidden with true on Entry [" . $liveEntry->getId() . "]");
            if (!$dryRun){
        ...
        {
            BorhanLog::debug("recordingOptions for entry [" . $liveEntry->getId() . "] is null - not changing it");
        }
    }

    BorhanLog::debug("need to update " . count($vodEntriesToUpdate) . " VOD entries. Using chunk size of " . UPDATE_VOD_CHUNK_SIZE);
...
{
    BorhanLog::debug("going to work on entries: " . implode($entryIds,","));
    ...
    {
        BorhanLog::debug("Starting to work on VOD Entry [" . $vodEntry->getId() . "]");
        if (!$dryRun)
        {
