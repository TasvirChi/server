...
if ( ! $desired_mode ) $desired_mode = 3 ; // BDP events from ps2 & ps3	
...
*/
	...
	$s = fgets($f);
	// collect statistics for old and new BDP
	...
	elseif ( strstr($s, "action=bmcCollect") && strstr($s, "service=stats") )  $mode = 4;	// ps3 bmcCollect stats
	...
	elseif  ( $mode == 4 && ($desired_mode&4)  )
	{
		$ps3_bmc_lines ++;
		// create event lines for ps3
		$event = new kEvent ( $vars , "bmcEvent" );
		
		$ks = @$vars["ks"];
		
		$eventLine = 
			$event->clientVer . EVENT_LOG_SEPARATOR 
			. $event->bmcEventType  . EVENT_LOG_SEPARATOR
			. $formatted_date . EVENT_LOG_SEPARATOR   // use server time
			. $event->bmcEventActionPath  . EVENT_LOG_SEPARATOR
		...
		// write to the the bmcEvents log NOT the bdpEvents log
	}
	else
	{
		$ignored_lines++;
...
fprintf($stderr, PHP_EOL . "total_lines [$total_lines] ps2_lines [$ps2_lines] ps3_lines [$ps3_lines] ps3_bmc_lines [$ps3_bmc_lines] ignored_lines [$ignored_lines]" . PHP_EOL );
