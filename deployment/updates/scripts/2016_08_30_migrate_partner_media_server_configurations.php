...
BorhanLog::debug("Starting partner media server config migration");

$partnerToWorkOn = getPartnersToWorkOn();
if(!count($partnerToWorkOn))
{
	BorhanLog::debug("No partners found to work on, Done!!!");
	return;	
}
...
foreach ($partnerToWorkOn as $partner) 
{
	...
	if($hdsDomain)
	{
		BorhanLog::debug("Found hds domain config for partner [{$partner->getId()}] value [$hdsDomain]");
	...
	if($hlsDomain)
	{
		BorhanLog::debug("Found hls domain config for partner [{$partner->getId()}] value [$hlsDomain]");
	...
	{
		BorhanLog::debug("Could not locate both HLS & HDS delivery profile ids continue to next partner");
		continue;
	}
	...
	BorhanLog::debug("Setting live delivery profile config: " . print_r($liveDeliveryProfileConfig, true) . " for partnerId [{$partner->getId()}]");
...
BorhanLog::debug("Done partner media server config migration");
