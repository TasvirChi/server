 ...
 *  --no-create
 *
 * to define user (defaults to apache) or group (defaults to borhan) for files ownership add
 *  --user={username}
 *  --group={groupname}
 */
...
//$argv = array( 1=> "--ini=c:/web/flash/bmc/v4.0.4/config.ini", 2 => "--no-create"); //used to teswt inside the zend studio
...
class uiConfDeployment
{
	...
	public static function deploy(Zend_Config_Ini $confObj)
	{
		...
		//Iterate through all sections (statics, general, bmc, bcw...)
		foreach ($confObj as $sectionName=> $sectionValue)
		{
			//If we are in the widgets section (like bmc, bcw, kse)
			if($sectionName != "general" && count($sectionValue->widgets))
			{
				...
				foreach ($sectionValue->widgets as $widgetName => $widgetValue)
				{
					...
					if($uiConf) //If the ui conf was generated successfully
					{
						...
						BorhanLog::debug("creating uiconf [$uiconf_id] for widget $widgetName with default values ( $baseSwfUrl , $swfName , $objectType ) for partner " . self::$partnerId);
						BorhanLog::debug("$widgetName , $baseSwfUrl , $swfName , $objectType");
						...
						if(isset($widgetValue->dependencies))
						{
							...
							foreach($widgetValue->dependencies as $dependencyName => $dependencyValue)
							{
								...
								else
								{
									...
									BorhanLog::debug("Missing dependency: {$dependencyName} = {$dependencyValue} for widget: {$widgetName}. Attempting to replace the token in uiconf features file");
								}
							}
						}
					...
					{
						BorhanLog::debug("failed to create uiconf object ($widgetName) due to missing values. check your config.ini");
					}
				}
			}
		}
	...
	public static function deprecateOldUiConfs($tag)
	{
		...
		foreach ($uiConfs as $oldUiConf)
		{
			...
			BorhanLog::debug("uiConf number {$oldUiConf[0]} was updated with the tag = {$deprecatedTag}");
			
			$totalDepractedCount += $deprecatedCount;
		}
		
		//Add the status check to the select factor
		BorhanLog::debug("{$totalDepractedCount} uiConfs were updated");

		$count = uiConfPeer::doCount($oldConfCriteria);
		
		if($count > 0)
		{
			BorhanLog::debug("Exiting, Tag: {$newTag} already found in the DB");
			exit;
		}
	}
	...
	public static function addUiConfThroughPropel(uiConf $pe_conf)
	{
		...
		if($pe_conf->getConfFile())
		{
			...
			if($ret !== 0 && $ret !== 127)
			{
				BorhanLog::debug("chown [$user_group] failed on path [$localPath] returned value [$ret]");
				exit(1);
			}
		}
	...
	public static function populateUiconfFromConfig($widget, $baseSwfUrl, $swfName, $objType, $disableUrlHashing)
	{
		...
		if($widget->conf_file)
		{
			...
			if(!$confFileContents)
			{
				BorhanLog::debug("Unable to read xml file from: {$widget->conf_file}");
			}
		
			if ($disableUrlHashing)
			{
				$confFileContents = str_replace('<Plugin id="borhanMix"','<Plugin id="borhanMix" disableUrlHashing="true" ',$confFileContents);
			}
		...
		if ($widget->config)
			...
			{
				BorhanLog::debug("Unable to read json file from: {$widget->config_file}");
			}
		...
		{
			BorhanLog::debug("missing features conf file for uiconf {$widget->name}"); // conf file is a must, features is not.
		}
		...
		$uiconf->setDisplayInSearch(mySearchUtils::DISPLAY_IN_SEARCH_BORHAN_NETWORK);
	...
	public static function updateUIConfFile(uiConf $uiconf, $newValue, $replacementString)
	{
		...
		if($ret !== 0)
		{
			BorhanLog::debug("chown [$user_group] failed on path [$localPath]");
			exit(1);
		}
	}
	...
	public static function printUsage($message)
	{
		...
		echo "    --group={groupname}: define group (defaults to borhan) for files ownership add\n";
		die;
	}
	...
	public static function checkArguments(array $arguments)
	{
		...
		{
			BorhanLog::debug("--partner argument wasn't given. Using defualt partner 0");
		}
		else
		{
...
class uiConfDeploymentCodeGenerator
{
	...
	public static function generateCode()
	{
		...
		$code[] = '$c->addAnd(UiConfPeer::TAGS, "%'.uiConfDeployment::$baseTag.'_".$this->bmc_'.uiConfDeployment::$baseTag.'_version."%", Criteria::LIKE);';
