...
//		list($silenceDetect, $blackDetect) = self::checkForSilentAudioAndBlackVideo($this->cmdPath, $this->filePath, $mediaInfo);
		...
		BorhanLog::log(print_r($mediaInfo,1));
	 ...
	 * @return BorhanMediaInfo
	 */
	protected function parseFormat($format, BorhanMediaInfo $mediaInfo)
	{
	 ...
	 * @return BorhanMediaInfo
	 */
	protected function parseStreams($streams, BorhanMediaInfo $mediaInfo)
	{
	$vidCnt = 0;
	$audCnt = 0;
	$dataCnt = 0;
	 ...
	 * @return BorhanMediaInfo
	 */
	protected function parseVideoStream($stream, BorhanMediaInfo $mediaInfo)
	{
	 ...
	 * @return BorhanMediaInfo
	 */
	protected function parseDataStream($stream, BorhanMediaInfo $mediaInfo)
	{
	 ...
	 */
	public static function checkForSilentAudioAndBlackVideo($ffmpegBin, $srcFileName, BorhanMediaInfo $mediaInfo, $detectDur=null)
	{
		BorhanLog::log("contDur:$mediaInfo->containerDuration,vidDur:$mediaInfo->videoDuration,audDur:$mediaInfo->audioDuration");
		...
		if(empty($detectMsg))
			BorhanLog::log("No black frame or silent content in $srcFileName");
		else
			BorhanLog::log("Detected - $detectMsg, in $srcFileName");
	 ...
	 * @return boolean
	 */
	public static function checkForGarbledAudio($ffmpegBin, $srcFileName, BorhanMediaInfo $mediaInfo)
	{
		BorhanLog::log("contDur:$mediaInfo->containerDuration,audDur:$mediaInfo->audioDuration");
		...
		if($audDetectDur>0 && $audDetectDur<10){
			BorhanLog::log("Audio OK - short audio, audDetectDur($audDetectDur)");
			return false;
		}
		...
		if($ticks<=10){
			BorhanLog::log("Audio OK - low numbers of ticks($ticks)");
			return false;
		}
		
		BorhanLog::log("audDetectDur($audDetectDur),ticks($ticks)");
		if($audDetectDur>0) {
			$ticksPerMin = $ticks/($audDetectDur/60);
			BorhanLog::log("ticksPerMin($ticksPerMin)");
			
			if($ticksPerMin<15 
			||($audDetectDur<60 && $ticksPerMin<30) 
			||($audDetectDur<120 && $ticksPerMin<20) ){
				BorhanLog::log("Audio OK");
				return false;
			}
		}
		else if($ticks<100) {
			BorhanLog::log("Audio OK - no duration, number of ticks smaller than threshold(100)");
			return false;
		}
		
		BorhanLog::log("Detected garbled audio.");
		return true;
	}
	...
	{
		//		BorhanLog::log("checkSilentAudioAndBlackVideo(contDur:$mediaInfo->containerDuration,vidDur:$mediaInfo->videoDuration,audDur:$mediaInfo->audioDuration)");
		...
		*/
		...
		if(empty($detectFiltersStr)){
			BorhanLog::log("No duration values in the source file metadata. Cannot run black/silence detection for the $srcFileName");
			return null;
		}
		...
		BorhanLog::log("Black/Silence detection cmdLine - $cmdLine");
		...
		*/
		...
		if($rv!=0) {
			BorhanLog::err("Black/Silence detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
	...
	{
		BorhanLog::log("srcFileName($srcFileName)");
		...
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			BorhanLog::err("SceneCuts detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
		/*
	...
	public static function retrieveKeyFrames($ffprobeBin, $srcFileName,$start=null,$duration=null)
	{
		...
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			BorhanLog::err("Key Frames detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return null;
		}
		return $outputArr;
	}
...
*/
		...
		BorhanLog::log("ScanType detection cmdLine - $cmdLine");
		$lastLine=exec($cmdLine , $outputArr, $rv);
		if($rv!=0) {
			BorhanLog::err("ScanType detection failed on ffmpeg call - rv($rv),lastLine($lastLine)");
			return 0;
		}
		...
		if($progessive<$tff+$bff)
			$scanType = 1;
		BorhanLog::log("ScanType: $scanType");
		return $scanType;
	}
...
*/
		...
		BorhanLog::log("FastStart detection cmdLine - $cmdLine");
		$lastLine=exec($cmdLine, $outputArr, $rv);
		{
			BorhanLog::log("FastStart detection results printout - lastLine($lastLine),output-\n".print_r($outputArr,1));
		}
		$fastStart = 1;
		foreach($outputArr as $line){
		...
		BorhanLog::log("FastStart: $fastStart");
		return $fastStart;
/*		
